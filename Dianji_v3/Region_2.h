...
int Run_Output_2(IloEnv env, int it,float lamda[], float Pipe[][25])
{
	env.out() << "--------------------------- Region 2 ------------------------ " << endl;
	//**********定义全局变量************
	IloInt cycle;			
	IloInt thUnitNum;  
	IloInt windUnitNum;
 ...
 //#define声明以及初始化符号常量；此外还可用关键字const unsigned float OUTFILEDATA="check.dat"声明
	...
	const char* lp = "./Bus31_2/hydrothermal.lp" ;    
	const char* SYSTEMDATA= "./Bus31_2/SystemData.dat" ;  //cycle J horizon flk tlk
	const char* APPDATA ="./Bus31_2/AppData.dat";     //I,K,LAMDA,PR DISK
	const char* THUNITDATA= "./Bus31_2/thunitdata.dat";
	const char* WINDDATA="./Bus31_2/windpower.dat";
	const char* NETDATA= "./Bus31_2/netdata.dat";
	const char* GAMADATA ="./Bus31_2/gamadata.dat";
	string _output = "./Bus31_2/Result_Output_"+int2str(it)+".dat";
	string _thunit = "./Bus31_2/Result_ThUnit_"+int2str(it)+".dat";
	string _result = "./Bus31_2/Result_"+int2str(it)+".dat";
	string _check = "./Bus31_2/Check_"+int2str(it)+".dat";
	...
	const char* Result_Line="./Bus31_2/Result_Line.dat";
	...
	try
	{
		...
		*/
		...
		for(s=0;s<outputNum;++s){
			...
			_outputR[s]=IloNumVarArray(env,cycle+1,0,outputMaxPower[s][1],ILOFLOAT);
			_outputRN[s]=IloNumVarArray(env,cycle+1,0,outputMaxPower[s][1],ILOFLOAT);
		}
		...
		for(t=1;t<cycle+1;t++)
		{
			...
			for(s=0;s<outputNum;++s)
			{
				...
				//model.add( outputPower[s][t] >= Pipe[s][t] );
